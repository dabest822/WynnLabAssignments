...
var speed = 300  # Horizontal movement speed
...
# Called every frame
func _process(delta):
	# Handle the attack (swing sword) when the Enter key is pressed
	if Input.is_action_just_pressed("attack") and not is_attacking:  # Enter key mapped to "attack"
		is_attacking = true

		# Play the hit animation based on the facing direction
		if facing_direction == "right":
			$AnimationPlayer.play("Hit_Right")  # Play sword swing right
		elif facing_direction == "left":
			$AnimationPlayer.play("Hit_Left")  # Play sword swing left
	...
	# After the attack is done, allow movement again
	if is_attacking and !$AnimationPlayer.is_playing():
		is_attacking = false  # Attack is done, allow movement again
